...
`target_class` defaults to `None`; pass an explicit class (e.g. `1`) to choose the outcome to flip rows into. Control the search budget with `n_trials` and `max_steps`:
