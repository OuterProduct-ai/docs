...
`LocalDataset.from_csv` and `LocalDataset.from_parquet` accept **CSV** and **Parquet** files.

`.upload()` always serializes the rows to Parquet before staging them, so the `parquet` extra — which provides `pyarrow` — is required for the upload workflow regardless of whether you started from a CSV or a Parquet file (reading a Parquet input with `from_parquet` needs it too):
