...
`predict()` requires an `op.Dataset` — you cannot pass a raw DataFrame or array directly. Wrap your inference rows before calling the model.
...
```python
# From a DataFrame
test_dataset = op.Dataset(df)

# From a CSV
test_dataset = op.Dataset.from_csv("new_customers.csv")

# From NumPy
test_dataset = op.Dataset.from_numpy(X_new)
```
