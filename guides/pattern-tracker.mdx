...
`op.reasoning.pattern_tracker.fit()` takes a `ReasoningModel`, a `DataFrame`, and a `target_range` that defines which prediction band to analyze. The `DataFrame` is a query over a table in your workspace, built with `ws.table(...)` or `ws.sql(...)`.
