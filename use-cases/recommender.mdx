---
title: "Add Reasoning to a Recommender System with OuterProduct"
sidebarTitle: "Recommender"
...
# workspace's values; `credential_name` references a Databricks
...
connector = op.DatabricksConnector(
    ...
    catalog="<catalog>",
    schema_name="<schema>",
    credential_name="marketplace-prod",
)
...
Marketplaces see a constant stream of users browsing for products, each scored on arrival. Pull the day's batch from your warehouse using the same feature schema as training — the view the production recommender already reads — and pass it through the reasoning engine. Predictions come back paired in real time with the rich context that powers every level of the app.
...
user = op.Dataset(users_today.iloc[[42]])
...
`get_global_drivers()`, `pattern_tracker.fit()`, `predict_and_explain()`, and `scenario()` compose freely in a custom app. Plug them into your enterprise stack — warehouses, BI tools, rule engines, and marketing-automation systems — and surface reasoning wherever it's needed: a Head of Growth dashboard, a partner-portal alert when a cohort's approval profile shifts, a "your fit for this card" panel inside the marketplace UI, or an LLM agent drafting user-facing advice.

<Warning>
  Enrich the reasoning context further with signals from your wider enterprise data, complementary models, and product-partner approval criteria to get the most out of each level.
</Warning>
