---
title: "Build an Underwriting Reasoning App"
sidebarTitle: "Underwriting"
icon: "file-invoice-dollar"
description: "Bolt traceable reasoning onto your risk-scoring model for audit trails, policy optimization, and per-applicant counterfactuals."
---
...
<Steps>
  <Step title="Distill your scoring model">
  ...
  <Step title="Train your reasoning engine">
    ...
    # Historical applicants (for training) live in Databricks. The
    # locator is catalog.schema.table; `connector` references a Databricks
    # credential you've already added in console.outerproduct.com.
    ws.register_databricks(
        "applicants",
        "main.underwriting.applicants_train",
        connector="underwriting-prod",
    )
    train = ws.table("applicants")
  ...
  <Step title="Plug in the daily applicant stream">
  ...
  <Step title="Plug in the daily applicant stream">
    ...
    ```python Score today's applicants
    today = ws.table("applicants").filter(op.col("received_date") == "2026-06-15")
    predictions, reasoning = reasoning_model.predict_and_explain(today)
    ```
  ...
  <Step title="Reason at three altitudes">
    ...
    Fit a `PatternTracker` on the denial band to surface the dominant _combinations_ of features that drive risk decisions. Each pattern is an executable filter with precision and lift stats, ready to apply to fresh applicants.
  ...
  <Step title="Reason at three altitudes">
    ...
    ```python Per-applicant explanation and counterfactual
    applicant = ws.table("applicants").filter(op.col("applicant_id") == "A-10042")
