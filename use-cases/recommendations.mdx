...
<Steps>
  ...
  <Step title="Train your reasoning engine">
    Register historical user data into a workspace, then train a reasoning engine on it with `op.reasoning.fit`. The workspace is your single data entry point, and its query engine (based on [Apache DataFusion](https://datafusion.apache.org/)) builds the `DataFrame` you train on. OuterProduct trains a reasoning engine alongside your recommender and leaves the production recommendation path untouched.

    <CodeGroup>

    ```python Train the reasoning engine
    import outerproduct as op
    
    op.init()
    ws = op.Workspace()
    
    # Historical users and approval outcomes (for training) live in
    # Databricks. The locator is catalog.schema.table; `connector`
    # references a Databricks credential you've already added in
    # console.outerproduct.com.
    ws.register_databricks(
        "recommendations",
        "main.marketplace.recommendations_train",
        connector="marketplace-prod",
    )
    train = ws.table("recommendations")
    ...
    <Tip>
      See the [Connectors guide](/guides/connectors) for Databricks, Snowflake, and S3 sources, and the [Training guide](/guides/training) for the full search and metric options.
    </Tip>
  </Step>
  ...
  <Step title="Plug in the live user stream">
    Marketplaces see a constant stream of users browsing for products, each scored on arrival. Query the day's batch from the same source table you trained on, so the scoring rows are built the same way as the training set, then pass that `DataFrame` through the reasoning engine. Predictions come back paired in real time with the rich context that powers every level of the app.

    <CodeGroup>

    ```python Score today's users
    today = ws.table("recommendations").filter(op.col("session_date") == "2026-06-15")
    predictions, reasoning = reasoning_model.predict_and_explain(today)
    ```
  ...
  <Step title="Reason at three altitudes">
    ...
    ```python Fit and apply a PatternTracker
    pt = op.reasoning.pattern_tracker.fit(
        reasoning_model, train, target_range=(0.5, None)
    ).wait()
    for fp in pt.patterns:
        print(f"{fp.label}: precision={fp.precision:.2f} lift={fp.lift:.2f}")
    ...
    ```python Per-user explanation and counterfactual
    user = ws.table("recommendations").filter(op.col("user_id") == "U-10042")
