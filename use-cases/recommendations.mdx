...
<Steps>
  <Step title="Distill your approval-odds model">
    Register historical user data into a workspace, wrap your approval-odds endpoint as a `Predictor`, and pass it as `teacher` to `op.reasoning.fit`. The workspace is your single data entry point, and its query engine (based on [Apache DataFusion](https://datafusion.apache.org/)) builds the `DataFrame` you train on. OuterProduct trains a reasoning engine alongside your recommender and leaves the production recommendation path untouched.
