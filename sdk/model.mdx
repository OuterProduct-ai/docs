...
<ParamField body="dataset" type="Dataset" required>
  An `op.Dataset` wrapping your inference rows. Build one with `op.Dataset(df)`, `op.Dataset.from_csv(...)`, or `op.Dataset.from_numpy(...)`. Column names must match the features seen during training.
</ParamField>
...
queries = op.Dataset(pd.DataFrame([
...
`Predictor` wraps an arbitrary HTTP scoring endpoint so you can use an existing model as a **teacher** in OuterProduct's distillation flow. Pass a `Predictor` as the `teacher` argument to `op.reasoning.fit()` to distil a `ReasoningModel` that mimics the teacher's predictions while adding full explainability.
...
model = op.reasoning.fit(
    dataset,
    teacher=teacher,   # label_column can be omitted when a teacher is provided
    task_type="binclass",
).wait()
...
<Note>
  When a `teacher` is provided, `label_column` is optional — OuterProduct queries the teacher to generate training labels. You can still supply `label_column` if you want to blend ground-truth labels with teacher predictions.
</Note>
