...
<ParamField body="dataset" type="Dataset" required>
  Training data wrapped in an `op.Dataset`. Build one with `op.Dataset(df)`, `op.Dataset.from_csv(...)`, or any other `Dataset` constructor. The column you name in `task`'s `label_column` (or the labels supplied by `teacher`) is used as the target; all remaining columns are treated as features.
</ParamField>

<ParamField body="task" type="TaskConfig | None">
  The supervised learning task, which carries the target `label_column`. Pass the config that matches your problem:

  | Config | Use when |
  |---|---|
  | `op.BinclassConfig(label_column=...)` | Binary classification (e.g. churn, fraud, approval) |
  | `op.MulticlassConfig(label_column=...)` | Multi-class classification (three or more discrete classes) |
  | `op.RegressionConfig(label_column=...)` | Continuous numerical target |
  | `op.ForecastingConfig(label_column=..., id_column=..., timestamp_column=..., horizon=..., lookback=...)` | Time-series forecasting |
  | `op.SequenceConfig(label_column=..., id_column=..., timestamp_column=...)` | Sequence modeling |
...
model = op.reasoning.fit(
    dataset,
    task=op.BinclassConfig(label_column="loan_default"),
