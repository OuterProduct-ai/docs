...
description: "API reference for the agentic documents module — convert unstructured documents into typed tabular features ready for ReasoningModel training."
---

The `outerproduct.agentic` module provides agent-driven pipelines that turn unstructured or semi-structured document sources into typed tabular features. The resulting datasets are fully compatible with `op.reasoning.fit()` and `Trainer.configure()`, so you can move from raw documents to a trained `ReasoningModel` without any manual feature engineering.
...
# 4. Train a ReasoningModel on the resulting tabular dataset
...
Applies a `DocumentSchema` to a collection of documents and returns a `DocumentDataset` — a typed tabular dataset where each row corresponds to one document and each column corresponds to a `DocumentQuestion` in the schema.
...
<ResponseField name="DocumentDataset" type="DocumentDataset">
  A typed tabular dataset derived from the source documents. Each row represents one document; each column represents one answered `DocumentQuestion`. Extends `Dataset`, so it can be passed directly to `op.reasoning.fit()` or `Trainer.configure()`.
</ResponseField>
