...
<ParamField body="questions" default="None" type="list[DocumentQuestion] | None">
...
<ResponseField name="DocumentDataset" type="DocumentDataset">
  A typed structured dataset derived from the source documents. Each row represents one document; each column represents one answered `DocumentQuestion`. You pass it directly to `op.reasoning.fit()` or `op.Trainer.configure()`, the same as a workspace `DataFrame`.
</ResponseField>
...
<ParamField body="column_name" type="str" required>
  The name to assign to the output column in the tabularized dataset (e.g. `"contract_value_usd"`). Use lowercase snake\_case for best compatibility with downstream tooling.
</ParamField>
...
A typed tabular dataset produced by `tabularize()`. You pass it directly to `op.reasoning.fit()` or `op.Trainer.configure()`, the same way you pass a `DataFrame` built from a workspace.

```python
dataset = documents.tabularize(raw_documents, schema=schema)

# Pass it straight to fit, like a workspace DataFrame
model = op.reasoning.fit(
    dataset,
    task=op.Binclass(label_column="has_auto_renewal"),
).wait()
...
<ResponseField name="DocumentDataset" type="DocumentDataset">
  A dataset handle over the extracted table. Each row corresponds to one source document; each column corresponds to one `DocumentQuestion` in the schema used to create it.
</ResponseField>

<Note>
  `tabularize()` returns a dataset you can hand straight to `op.reasoning.fit()` or `op.Trainer.configure()`. Training and reasoning then work the same as for a workspace `DataFrame`.
</Note>