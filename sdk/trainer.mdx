...
trainer = op.Trainer.configure(dataset, task=op.BinclassConfig(label_column="approved"))
...
trainer = op.Trainer.configure(
    dataset,
    task=op.BinclassConfig(label_column="approved"),
    model_types=["tabm", "xgboost"],
    metric=op.Metric("auc"),
)
