...
description: "Stage local structured data with outerproduct.LocalDataset (CSV, Parquet, pandas, NumPy) and upload it to a server-backed Dataset for training and inference."
---
...
<Note>
  `op.LocalDataset` also accepts a polars DataFrame via `from_polars(df)`, plus `torch.Tensor`, 2-D `numpy.ndarray`, and nested lists.
</Note>
