...
<Note>
  The target (label) column is **not** declared on the dataset. You specify it later using the `task` config (e.g. `op.Binclass(label_column=...)`) in [`Trainer.configure()`](/sdk/trainer) or `op.reasoning.fit()`.
</Note>
